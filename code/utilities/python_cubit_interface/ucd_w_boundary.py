...
# Original version by Kevin Pendley and Boris Vexler.
# Improved version cleaned and extended to support boundary 
# conditions by Luca Heltai
...
if n_hex_cells == 0:
  ...
  for bc_id in bc_ids :
    ...
    for bc_curve in bc_curves[bc_id]:
      cubit.cmd("group 'temp_bc_curves' add edge all in curve " +
str(bc_curve))
    group_id = cubit.get_id_from_name("temp_bc_curves")
    bc_edges[bc_id] = cubit.get_group_edges(group_id)
    cubit.cmd("delete group " + str(group_id))
    n_bc_edges += len(bc_edges)
