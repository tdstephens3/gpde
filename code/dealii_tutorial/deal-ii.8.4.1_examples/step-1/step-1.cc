...
void first_grid ()
{
  ...
  std::ifstream input_file (in_mesh_filename);
  ...
  //std::ofstream out_vtk ("grid_ellipsoid_1.vtk");
  std::ofstream out_vtk ("grid_ellipsoid_1.vtk");
  
  GridOut grid_out;
  grid_out.write_vtk (triangulation, out_vtk);
  std::cout << "Grid written to grid_ellipsoid_1.vtk" << std::endl;
  ...
  //// cells in total:
  //const std::vector<unsigned int> holes = {1, 3};
