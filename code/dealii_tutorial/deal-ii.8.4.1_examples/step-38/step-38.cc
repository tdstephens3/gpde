...
// function definitions for RightHandSide class
  ...
  /*{{{*/
    ...
    const std::string cad_file_name = "ellipsoid_manifold.iges";
    TopoDS_Shape manifold = OpenCASCADE::read_IGES(cad_file_name, 1);

    const double tolerance = OpenCASCADE::get_shape_tolerance(manifold) * 5;
    ...
    OpenCASCADE::extract_compound_shapes(manifold,
                                         compounds,
                                         compsolids,
                                         solids,
                                         shells,
    ...
    triangulation.set_manifold(2, line_projector);
     ...
     static OpenCASCADE::NormalProjectionBoundary<2,3> normal_projector(manifold, tolerance);
