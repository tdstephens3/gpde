...
class LaplaceBeltramiProblem
{
  ...
  private:
    ...
    Vector<double>                scalar_mean_curvature;
...
void LaplaceBeltramiProblem<spacedim>::solve ()
{
  ...
  scalar_mean_curvature.reinit (dof_handler.n_dofs());
  
  
  for (unsigned int i=0; i<dof_handler.n_dofs(); ++i )
  {
    scalar_mean_curvature(i) = solution_x(0) + solution_y(1) + solution_z(2);
  }
  /*}}}*/
}
...
void LaplaceBeltramiProblem<spacedim>::output_results () const
{
  ...
  data_out.add_data_vector (scalar_mean_curvature,
  ...
  std::string filename ("./data/scalar_mean_curvature-");
...
void LaplaceBeltramiProblem<spacedim>::compute_error (double a, double b, double c, Point<3> center) const
{
  ...
  VectorTools::integrate_difference (mapping, dof_handler, scalar_mean_curvature,
